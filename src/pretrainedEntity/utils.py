...
def convert_dataset_to_samples(
    dataset, max_span_length, ner_label2id=None, context_window=0, split=0
):
    """
    ...
    for c, doc in enumerate(dataset):
        ...
        for i, sent in enumerate(doc):
            ...
            if context_window != 0 and len(sent.text) > context_window:
                logger.info(
                    "Long sentence: {} {}".format(sample, len(sent.text))
                )
    ...
    avg_length = sum([len(sample["tokens"]) for sample in samples]) / len(
        samples
    )
